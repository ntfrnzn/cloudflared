...
func (s *Supervisor) startFirstTunnel(ctx context.Context, connectedSignal chan struct{}) {
	err := ServeTunnelLoop(ctx, s.config, s.config.Metrics, s.getEdgeIP(0), 0, connectedSignal)
	defer func() {
...
func (s *Supervisor) startTunnel(ctx context.Context, index int, connectedSignal chan struct{}) {
	err := ServeTunnelLoop(ctx, s.config, s.config.Metrics, s.getEdgeIP(index), uint8(index), connectedSignal)
